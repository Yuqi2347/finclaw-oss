...
## FinClaw OSS capability integration

- 作为 FinClaw 顶层 `capabilities/tradingagents_astock` 能力随仓库分发。
- FinClaw 启动个股研究时会显式传入 `TRADINGAGENTS_DATAHUB_URL` / `FINDATAHUB_API_BASE`，使研究链路复用 FinClaw 当前 DataHub 服务。
- HTML 报告导出改为 capability 内置静态 renderer，不再依赖 BettaFish ReportEngine 或任何 GPL 代码。
- 运行目录、缓存目录、记忆文件默认落在 capability 自身 `runtime/` 下，并由 FinClaw OSS `.gitignore` 排除。
