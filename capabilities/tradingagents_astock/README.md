...
> FinClaw OSS 说明：本目录作为 FinClaw 的 `TradingAgents AStock` capability 分发。FinClaw 启动该能力时会注入当前 DataHub 地址，并使用本目录内置的静态 HTML renderer 生成报告，不依赖 BettaFish ReportEngine。
...
| 维度 | 原版 | 本 Fork |
|------|------|---------|
| **数据源** | Yahoo Finance / Alpha Vantage | FinDataHub 统一数据层（FinClaw 默认使用 Tushare profile） |
...
FinClaw OSS 集成版统一通过 FinDataHub 取数，TradingAgents 不直接访问上游行情库：

| 来源 | 协议 | 提供内容 |
|------|------|---------|
| **FinDataHub** | HTTP / local embedded service | K 线、财务、估值、新闻、资金、市场包 |
| **Tushare profile** | FinDataHub provider | FinClaw 默认付费/稳定数据链路 |

> TradingAgents 只调用 `tradingagents.dataflows.a_stock`，由 FinDataHub 负责 provider 选择、缓存、补数和降级。
