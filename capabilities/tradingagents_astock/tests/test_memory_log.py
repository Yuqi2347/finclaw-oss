...
class TestDeferredReflection:
    ...
    def test_fetch_returns_valid_ticker(self):
        ...
        with patch("tradingagents.graph.trading_graph.datahub_forward_return", return_value=(0.06, 5)):
    ...
    def test_fetch_returns_too_recent(self):
        ...
        mock_graph = MagicMock(spec=TradingAgentsGraph)
        with patch("tradingagents.graph.trading_graph.datahub_forward_return", side_effect=ValueError("not enough data")):
    ...
    def test_fetch_returns_delisted(self):
        ...
        mock_graph = MagicMock(spec=TradingAgentsGraph)
        with patch("tradingagents.graph.trading_graph.datahub_forward_return", side_effect=ValueError("empty data")):
            raw, alpha, days = TradingAgentsGraph._fetch_returns(mock_graph, "XXXXXFAKE", "2026-01-10")
        assert raw is None and alpha is None and days is None

    def test_fetch_returns_uses_actual_datahub_days(self):
        """DataHub decides how many forward trading days are available."""
        mock_graph = MagicMock(spec=TradingAgentsGraph)
        with patch("tradingagents.graph.trading_graph.datahub_forward_return", return_value=(0.03, 2)):
            raw, alpha, days = TradingAgentsGraph._fetch_returns(mock_graph, "688017", "2026-01-05")
        assert raw == 0.03
        assert alpha is None
        assert days == 2
